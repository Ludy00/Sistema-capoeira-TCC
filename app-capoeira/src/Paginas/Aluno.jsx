...
function AlunoDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col md:flex-row font-sans">
      ...
      {/* === NAVBAR MOBILE CORRIGIDA (LINHA ÚNICA) === */}
      <div className="md:hidden bg-[#0e0e0e] border-b border-gray-800 p-4 flex justify-between items-center sticky top-0 z-50 shadow-2xl">
        <div className="flex items-center space-x-3">
          <img src="/logooficial.png" className="w-7 h-7 object-contain" alt="Logo" />
          <span className="text-white font-black uppercase tracking-tighter text-xs">Portal do Aluno</span>
        </div>
        <button 
          onClick={() => setSidebarAberta(!sidebarAberta)} 
          className="p-2 bg-gray-900 rounded-lg text-gray-400 hover:text-white transition-all"
        >
          {sidebarAberta ? <X size={20} /> : <Menu size={20} />}
        </button>
      </div>

      {/* === SIDEBAR (BARRA LATERAL COM Z-INDEX AJUSTADO) === */}
      <aside className={`
        fixed inset-y-0 left-0 z-[60] w-72 bg-[#111] border-r border-gray-800 transform transition-transform duration-300 ease-in-out
          ...
          <div className="mb-12 flex flex-col space-y-6">
            <div className="flex items-center space-x-3 opacity-60">
              <div className="w-8 h-8 rounded-lg bg-red-600/20 flex items-center justify-center border border-red-600/30">
                <img src="/logooficial.png" alt="Logo" className="w-5 h-5 object-contain" />
              </div>
              <span className="text-white font-bold uppercase tracking-widest text-[10px]">Muzenza PHB</span>
            </div>
            
            <div className="pl-1 text-left">
              <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em] mb-1">Área do Aluno</p>
          ...
          <nav className="flex-1 space-y-3">
            {menuItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center space-x-4 px-5 py-4 rounded-2xl font-bold transition-all text-sm
                  ${abaAtiva === item.id 
                    ? 'bg-red-700 text-white shadow-lg shadow-red-900/20 scale-[1.02]' 
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
              <div className="bg-[#111] rounded-[2rem] border border-gray-800 overflow-hidden relative group hover:border-red-900/40 transition-all duration-500">
                ...
                <div className="p-8">
                  ...
                  <h3 className="text-2xl font-black text-white mb-2 tracking-tight">Treino de Fundamentos</h3>
        ...
        {abaAtiva === 'mensalidade' && (
          <div className="max-w-2xl mx-auto space-y-8 animate-fadeIn text-center">
            <h2 className="text-2xl font-black text-white uppercase tracking-widest border-l-4 border-red-600 pl-4 text-left">Financeiro</h2>
            <div className="bg-[#111] p-12 rounded-[2.5rem] border border-gray-800 space-y-8 shadow-2xl">
              ...
              <div>
                <h3 className="text-3xl font-black text-white tracking-tighter">Tudo em Dia!</h3>
                <p className="text-gray-500 mt-3 text-lg">Sua mensalidade de Abril/2026 está paga.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {abaAtiva === 'configuracoes' && (
          <div className="max-w-2xl mx-auto space-y-8 animate-fadeIn text-center py-20">
             ...
             <p className="text-gray-500 max-w-sm mx-auto">Em breve você poderá personalizar seus dados aqui.</p>
          </div>
        )}

      </main>

      {/* OVERLAY MOBILE (Z-INDEX SUPERIOR) */}
      {sidebarAberta && (
        <div 
